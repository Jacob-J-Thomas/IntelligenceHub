using IntelligenceHub.Common.Attributes;

namespace IntelligenceHub.API.DTOs.DataAccessDTOs
{
    public class RagChunk
    {
        public int Id { get; set; }
        public string Title { get; set; }
        public string Content { get; set; }
        public string Topic { get; set; }
        public string KeyWords { get; set; }
        public int? Chunk { get; set; }
        public string SourceName { get; set; }
        public string SourceLink { get; set; }
        public int PermissionGroup { get; set; }
        public byte[] TitleVector { get; set; }
        public byte[] ContentVector { get; set; }
        public byte[] TopicVector { get; set; }
        public byte[] KeywordVector { get; set; }
        public float TitleVectorNorm { get; set; }
        public float ContentVectorNorm { get; set; }
        public float TopicVectorNorm { get; set; }
        public float KeywordVectorNorm { get; set; }
        public DateTime CreatedDate { get; set; }
        public DateTime ModifiedDate { get; set; }
        public int AccessCount { get; set; }
    }
}
